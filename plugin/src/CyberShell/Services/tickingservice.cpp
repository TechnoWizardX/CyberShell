...
TickingService::TickingService(QObject* parent)
    ...
    auto* dash = cybershell::config::GlobalConfig::instance()->dashboard();
    applyInterval(dash->resourceUpdateInterval());
    QObject::connect(dash, &cybershell::config::DashboardConfig::resourceUpdateIntervalChanged, this, [this, dash] {
        applyInterval(dash->resourceUpdateInterval());
    });
}
